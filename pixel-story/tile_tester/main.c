...
anim_file_t *a;
char no_texture[64] = {0x47, 0x49, 0x46, 0x38, 0x39, 0x61, 0x10, 0x00, 0x10, 0x00, 0x80, 0x00, 0x00, 0xFF, 0x00, 0xFF,
                       0x00, 0x00, 0x00, 0x2C, 0x00, 0x00, 0x00, 0x00, 0x10, 0x00, 0x10, 0x00, 0x00, 0x02, 0x1F, 0x84,
                       0x6F, 0xA1, 0xAB, 0x88, 0xCC, 0xDC, 0x81, 0x4B, 0x26, 0x1A, 0x2C, 0xC0, 0xD9, 0x72, 0xFD, 0x79,
                       0x1C, 0x06, 0x92, 0x22, 0x55, 0xA2, 0x27, 0x94, 0xB2, 0x6B, 0xF4, 0x56, 0x56, 0x01, 0x00, 0x3B};
...
int main(int argc, char *argv[]) {
    ...
    window = SDL_CreateWindow("Tile Tester", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, SCREEN_WIDTH,
    ...
    if (argc > 1) {
        ...
        if (argc > 2 && ani_init(render, argv[2], &a) == A_SUCCESS) {
            anim = 1;
        } else {
            printf("%s\n", SDL_GetError());
        }
    ...
    while (!quit_flag) {
        ...
        for (i = 0; i < FULL_WIDTH; i += TILE_STEP) {
            for (j = 0; j < FULL_HEIGHT; j += TILE_STEP) {
                pos.x = i + k_x - TILE_STEP;
                pos.y = j + k_y - TILE_STEP;
