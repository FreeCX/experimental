...
int ani_init( SDL_Renderer *r, char *config, anim_file_t **a )
{
    ...
    fclose( f );
    wnd.x = 0; wnd.y = 0; wnd.w = obj->frm_size; wnd.h = obj->frm_size;
    pos.x = 0; pos.y = 0; pos.w = obj->frm_size; pos.h = obj->frm_size;
    *a = obj;
    return A_SUCCESS;
}
...
int ani_draw( SDL_Renderer *r, anim_file_t *obj, int id, int x, int y )
{
    ...
    while ( id != obj->a[++i].id );
    p = &(obj->a[i]);
    wnd.x = p->frm_select * obj->frm_size; wnd.y = p->dy;
    pos.x = x; pos.y = y;
