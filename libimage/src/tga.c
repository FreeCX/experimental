...
const char tga_info_01[] = "tga.id_length   = %u\n"
                           "tga.color_map   = %u\n"
                           "tga.img_type    = %u\n"
                           "tga.entry_index = %u\n"
                           "tga.length      = %u\n"
...
void tga_info(tga_fmt_t *h) {
    printf(tga_info_01, h->id_length, h->color_map, h->img_type, h->entry_index, h->length, h->entry_size, h->x_origin,
           h->y_origin, h->img_width, h->img_height, h->img_depth, h->img_descriptor);
}
...
int8_t tga_load(FILE *f, image_t *img) {
    uint32_t xsize, ysize, image_size;
    uint8_t *data, depth;
    size_t i, j;
    tga_fmt_t h;
    ...
    switch (h.img_type) {
        ...
        case 3: // grey
            ...
            fread(data, sizeof(uint8_t), image_size, f);
            img->data = data;
            img->bpp = depth;
            img->width = xsize;
            img->height = ysize;
